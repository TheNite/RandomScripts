...
def enable_virtualbox():
    subprocess.call(
        "powershell.exe bcdedit /set hypervisorlaunchtype off", shell=True)
    subprocess.call(
        "powershell.exe dism.exe /online /disable-feature /featurename:Microsoft-Windows-Subsystem-Linux /norestart",
        shell=True,
    )
    subprocess.call(
        "powershell.exe dism.exe /online /disable-feature /featurename:VirtualMachinePlatform /norestart",
        shell=True,
    )
    reboot_windows()
...
def enable_wsl2():
    subprocess.call(
        "powershell.exe bcdedit /set hypervisorlaunchtype auto", shell=True)
    subprocess.call(
